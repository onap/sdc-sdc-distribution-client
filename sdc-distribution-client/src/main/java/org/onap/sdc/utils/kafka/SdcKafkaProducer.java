...
public class SdcKafkaProducer {
    ...
    private static void init(IConfiguration configuration) {
        ...
        props.put(SaslConfigs.SASL_JAAS_CONFIG, getJaasConf(configuration));
    ...
    private static String getJaasConf(IConfiguration configuration) {
        String saslJaasConfFromFile = configuration.getKafkaSaslJaasConfig();
        if (saslJaasConfFromFile != null) {
            return saslJaasConfFromFile;
        }
        String saslJaasConfFromEnv = System.getenv("JAASCONFIG");
